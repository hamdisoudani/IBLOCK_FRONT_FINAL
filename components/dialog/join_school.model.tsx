"use client";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Button } from "../ui/button";
import { Plus } from "lucide-react";
import { ErrorMessage, Field, Formik } from "formik";
import { Label } from "../ui/label";
import React, { useState } from "react";
import { joinProjectShema } from "@/lib/schemas/manage_project/join_project.schema";
import { InferType } from "yup";
import axiosInstance from "@/plugins/axios";
import { cn } from "@/lib/utils";
import { useToast } from "../ui/use-toast";
import { useProfileContext } from "../context/userprofile.context";



const JoinSchool = () => {
    const [isDialogOpen, setIsDialogOpen] = useState(false); // Added state
    const { toast } = useToast();
    const { getUserProfiles } = useProfileContext();

  const handleJoinNewProject = async (
    formData: InferType<typeof joinProjectShema>,
    { resetForm }: { resetForm: any }
  ) => {
    await axiosInstance
      .post("/school/join", {  
        invitationCode: formData.invitationCode,
      })
      .then(async (res) => {
        toast({
            title: "Success",
            description: "You have successfully joined the school",
            className: "bg-green-900",
            });
        await getUserProfiles();
      })
      .catch((e) => {
        // Check if the received error message is an array or a string
         if(Array.isArray(e.response.data.message)) {
            toast({
                title: "Error",
                description: e.response.data.message[0],
                className: "bg-red-900",
              });
         } else if(e.response.data.message) {
            toast({
                title: "Error",
                description: e.response.data.message,
                className: "bg-red-900",
              });
        } else {
            toast({
                title: "Error",
                description: "Something went wrong",
                className: "bg-red-900",
              });
        }
      })
      .finally(() => {
        setIsDialogOpen(false);
        resetForm();
      });
  };
  return (
      <Dialog open={isDialogOpen}>
          <DialogTrigger asChild> 
            <Button 
              variant={"ghost"}
              onClick={() => setIsDialogOpen(true)} 
              asChild
              className="flex justify-start"
            >
              <span 
                className="w-full inline-flex items-center gap-1 px-3 py-2 rounded-lg cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-600 dark:text-white" 
              >
                <Plus className="h-6 w-6 text-black stroke-1" />
                <p className="text-sm font-medium text-black">
                  Join school
                </p>
              </span>
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Type invitation code</DialogTitle>
              <DialogDescription>
                Anyone who has this code will be able to join school.
              </DialogDescription>
            </DialogHeader>
            {/* We'll add the Formik component in the next section */}
            <Formik
              initialValues={{ invitationCode: "" }}
              validationSchema={joinProjectShema}
              onSubmit={handleJoinNewProject} 
              validateOnBlur
              validateOnChange
              validateOnMount
            >
              {({
                values,
                handleChange,
                handleSubmit,
                errors,
                touched,
                handleBlur,
                isValid,
                dirty,
                isSubmitting,
              }) => (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="invitationCode">
                      Invitation code
                    </Label>
                    <Field
                      type="text"
                      className={cn(
                        "border border-black dark:border-white w-full p-2 rounded",
                        {
                          "border-[2px] border-red-800 dark:border-red-800 bg-red-200":
                            errors.invitationCode && touched.invitationCode,
                        }
                      )}
                      value={values.invitationCode}
                      onChange={handleChange}
                      onBlur={handleBlur}
                      name="invitationCode"
                      placeholder="Enter your project name"
                    />
                    <ErrorMessage
                      name="invitationCode"
                      component="div"
                      className="text-red-600"
                    />
                  </div>

                  <DialogFooter className="sm:justify-start space-y-2 gap-2">
                    <Button
                      className="w-full"
                      type="submit"
                      disabled={!isValid || isSubmitting}
                    >
                      <span className={cn({ hidden: isSubmitting })}>
                        Join School 
                      </span>
                      <svg
                        aria-hidden="true"
                        role="status"
                        className={cn(
                          "inline w-4 h-4 me-3 text-white animate-spin",
                          { hidden: !isSubmitting }
                        )}
                        viewBox="0 0 100 101"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                          fill="#E5E7EB"
                        />
                        <path
                          d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                          fill="currentColor"
                        />
                      </svg>
                    </Button>
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() => {
                        setIsDialogOpen(false);
                      }}
                    >
                      Close
                    </Button>
                  </DialogFooter>
                </form>
              )}
            </Formik>
          </DialogContent> 
    </Dialog>
  );
};

export default JoinSchool;