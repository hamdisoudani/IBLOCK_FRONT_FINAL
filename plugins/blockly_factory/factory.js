...
import { StandardCategories } from './standard_categories';
import "./blocks";
/**
 * Namespace for Block Factory.
 */
export const BlockFactory = Object.create(null);
...
BlockFactory.setGeneratedJsonCode = (json) => {};

BlockFactory.setGeneratedPythonCode = (pythonCode) => {};

BlockFactory.generatedJsonCode = "";
 ...
 */
BlockFactory.setBlockType = (blockType) => {};
BlockFactory.updateLanguage = function() {
  ...
  if (!BlockFactory.updateBlocksFlag) {
    ...
    BlockFactory.setGeneratedJsonCode(code);
    BlockFactory.updatePreview(code);
    BlockFactory.setBlockType(blockType);
  }
...
BlockFactory.updateGenerator = function(block) {
  ...
  console.log('generatorStub', generatorStub);
  BlockFactory.setGeneratedPythonCode(generatorStub);
 ...
 */
BlockFactory.setIsBlockTypeForbidden = ({error, message}) => {};
BlockFactory.updatePreview = function(dataCode = null) {
  ...
  if (BlockFactory.oldDir !== newDir) {
    ...
    var rtl = newDir === 'rtl';
    BlockFactory.previewWorkspace = Blockly.inject('previewDiv',
        {
  ...
  //var code = BlockFactory.generatedJsonCode;
  var code = dataCode || BlockFactory.generatedJsonCode;
  if (!code.trim()) {
  ...
  var warnExistingBlock = function(blockType) {
    if (blockType in Blockly.Blocks) {
      ...
      BlockFactory.setIsBlockTypeForbidden({error: true, message: text});
      return true;
    }
    BlockFactory.setIsBlockTypeForbidden(false);
    return false;
  }
  ...
  try {
    ...
    if (StandardCategories.coreBlockTypes.includes(blockType)) {
      BlockFactory.setIsBlockTypeForbidden({error: true, message: 'A core Blockly block already exists under this name.'});
      ...
      // name 'block_type'
      BlockFactory.setIsBlockTypeForbidden({error: true, message: 'You cannot save a block with the default name, "block_type"'});
      rootBlock.setWarningText('You cannot save a block with the default ' +
          'name, "block_type"');

    } else {
      BlockFactory.setIsBlockTypeForbidden({error: false, message: ''});
